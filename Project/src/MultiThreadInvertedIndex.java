...
public class MultiThreadInvertedIndex {
	...
	public void indexToJson(Path path) throws IOException {
		...
		try {
			...
			MultiThreadJsonWriter.asNestedObject(elements, path);
	...
	public void wordCountToJson(Path path) throws IOException{
		...
		try {
			...
			MultiThreadJsonWriter.wordCountsPrinter(counts, path);
	...
	/**
	 * Partial search method for checking the star-with word from query to MultithreadInvertedIndex
	 * @param queries  collection of query words
	 * @return an arrayList contains SearchResult to be added in map
	 */
	 ...
	 * @param matchedWord the matched word from query to MultithreadInvertedIndex
	 */
